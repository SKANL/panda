...
It is a LEAF. It depends on `@panda/contracts` and nothing else (AD-2), and it
...
(AD-7). `@panda/registry` and `@panda/projection` each translate those two codes
into their own at their own boundary.

This code was moved here from `@panda/registry`, where it had been the store's
