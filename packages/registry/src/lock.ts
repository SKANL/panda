...
// (no locking dependency). A lock is the file `<store>.lock`, created with
// O_EXCL semantics so exactly one contender wins per host, containing JSON
...
function currentHolder(): LockHolder {
  ...
  if (!Number.isSafeInteger(pid) || pid <= 0) {
    throw new PandaError(
      ...
      `cannot acquire registry lock: process pid ${pid} is not a positive integer`,
    )
  }
...
function contention(path: string, state: LockFileState): PandaError {
  if (state.kind === 'corrupt') {
    return new PandaError(
      ...
      `registry lock '${path}' names an unreadable lockfile (${state.reason}); will become breakable after the corrupt grace period`,
    )
  }
  const named =
  ...
  return new PandaError(
    PANDA_ERROR_CODES.registryContention,
    `registry lock '${path}' is held by ${named}; another panda process is mid-mutation`,
  )
}
...
function unavailable(operation: string, path: string, cause: unknown): PandaError {
  return new PandaError(
    PANDA_ERROR_CODES.registryStoreUnavailable,
    `registry lock ${operation} failed on '${path}': ${cause instanceof Error ? cause.message : String(cause)}`,
    { cause },
  )
}
