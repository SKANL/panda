...
| M4 | Panda already knows each vendor's container key, verified against the real binaries | `claude-mcp.ts:25` `mcpServers`, `codex-config.ts:22` `mcp_servers`, `opencode-config.ts:21` `mcp`. All three trait records are exported from `@skanl/panda-projection`. |
