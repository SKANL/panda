...
- **CI, the writability probe answered for a write panda does not perform (patch):** the fix above probed `access(W_OK)` on the target FILE, and panda has never written a file that way — every byte lands through one atomic writer that creates a temp file in the target's directory and RENAMES it over the target. Measured on Linux (WSL2 kernel and a Node 24 container, both as uid 1000): rename over a `0444` target inside a writable directory SUCCEEDS and the new bytes are there, while `access(0444, W_OK)` says false. So on every Linux and macOS machine the patch above reported `not-writable` on a file panda writes without complaint, and handed the user a location to fix that was never broken — this spec's own Never clause, inverted by the correction written to satisfy it. Windows is the one platform where the two agree: rename over a read-only file fails `EPERM` there, measured on this machine, and a `0555` directory blocks nothing at all. The probe now asks what rename asks — the directory on both platforms, plus the target's read-only attribute on win32 only — and both of its two callers (the vendor file, and panda's own ledger) go through that same writer, confirmed by grep rather than assumed. **CI is what caught it**, on the ubuntu runner under Node 24 and 26; the repo is developed on Windows, where the file check and the rename happen to give the same answer, so every local run was green. The two rows themselves were the deeper defect: they asserted a fixed finding kind on a precondition only one platform can produce, which is how one platform's semantics became the spec. They are DIFFERENTIAL now — each builds the state that genuinely blocks the write HERE (the directory on POSIX, the read-only attribute on win32), proves it by attempting the real temp+rename, and then asserts doctor agrees with the outcome that was OBSERVED; a state that cannot be produced (root, a mode-ignoring filesystem) skips with its reason instead of asserting something untrue. A third row was added for the state the two platforms disagree about — a `0444` file in a writable directory — and it asserts only agreement, never a kind. All three were run against the pre-fix probe on Linux and fail 3/3, and against the fix on Linux and on Windows and pass: 61/61 in `@skanl/panda-environment` on both.
