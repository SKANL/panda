...
- **Follow-on sandbox slice, scope boundary:** `ToolProvider` remains the discovery/ingestion port defined by this story. Sandbox `ToolExecutor` is a separate, explicit execution seam; a discovered entry does not confer execution authority.
