...
- **Language** — Speak in `Spanish`. Write any file output in `English`.
...
1. Explicit argument
   ...
   - If it points to a file that matches the spec template (has `status` frontmatter with a recognized value: draft, ready-for-dev, in-progress, in-review, or done) → set `spec_file`. Before exiting, run **Story-key resolution** (below). Then **EARLY EXIT** to the appropriate step: `draft` → `C:\code\panda\_bmad\render\bmad-build\panda-af65937e8dcd\b1d808c3bf9d40925d5a\step-02-plan.md`, `ready-for-dev`/`in-progress` → `C:\code\panda\_bmad\render\bmad-build\panda-af65937e8dcd\b1d808c3bf9d40925d5a\step-03-implement.md`, `in-review` → `C:\code\panda\_bmad\render\bmad-build\panda-af65937e8dcd\b1d808c3bf9d40925d5a\step-04-review.md`. For `done`, ingest as context and proceed to INSTRUCTIONS — do not resume.
...
3. Otherwise — scan artifacts and ask
   - Active specs (`draft`, `ready-for-dev`, `in-progress`, `in-review`) in `C:\code\panda/_bmad-output/implementation-artifacts`? → List them and HALT. Ask user which to resume (or `[N]` for new).
     - If `draft` selected: Set `spec_file`. Run **Story-key resolution** (below). **EARLY EXIT** → `C:\code\panda\_bmad\render\bmad-build\panda-af65937e8dcd\b1d808c3bf9d40925d5a\step-02-plan.md` (resume planning from the draft)
     - If `ready-for-dev` or `in-progress` selected: Set `spec_file`. Run **Story-key resolution** (below). **EARLY EXIT** → `C:\code\panda\_bmad\render\bmad-build\panda-af65937e8dcd\b1d808c3bf9d40925d5a\step-03-implement.md`
     - If `in-review` selected: Set `spec_file`. Run **Story-key resolution** (below). **EARLY EXIT** → `C:\code\panda\_bmad\render\bmad-build\panda-af65937e8dcd\b1d808c3bf9d40925d5a\step-04-review.md`
...
If the spec is an epic story and `C:\code\panda/_bmad-output/implementation-artifacts/sprint-status.yaml` exists: find the `development_status` key matching `{epic_num}-{story_num}` by exact numeric equality on the first two segments (so `1-1` never collides with `1-10`). Exactly one match → set `story_key` to that full key. Zero or multiple matches → leave `story_key` unset (warn on multiple).

## INSTRUCTIONS

1. Load context.
   - List files in `C:\code\panda/_bmad-output/planning-artifacts` and `C:\code\panda/_bmad-output/implementation-artifacts`.
   ...
   - **Determine context strategy.** Using the intent and the artifact listing, infer whether the current work is a story from an epic. Do not rely on filename patterns or regex — reason about the intent, the listing, and any epics file content together.
     ...
     **A) Epic story path** — if the intent is clearly an epic story:
     ...
     2. **Check for a valid cached epic context.** Look for `C:\code\panda/_bmad-output/implementation-artifacts/epic-<N>-context.md` (where `<N>` is the epic number). A file is **valid** when it exists, is non-empty, starts with `# Epic <N> Context:` (with the correct epic number), and no file in `C:\code\panda/_bmad-output/planning-artifacts` is newer.
     ...
     3. **Compile epic context.** Produce `C:\code\panda/_bmad-output/implementation-artifacts/epic-<N>-context.md` by following `C:\code\panda\_bmad\render\bmad-build\panda-af65937e8dcd\b1d808c3bf9d40925d5a\compile-epic-context.md`, in order of preference:
        - **Preferred — subagent:** spawn a subagent synchronously (wait for it to return in this turn) with `C:\code\panda\_bmad\render\bmad-build\panda-af65937e8dcd\b1d808c3bf9d40925d5a\compile-epic-context.md` as its prompt. Pass it the epic number, the epics file path, the `C:\code\panda/_bmad-output/planning-artifacts` directory, and the output path `C:\code\panda/_bmad-output/implementation-artifacts/epic-<N>-context.md`.
        - **Fallback — inline** (for runtimes without subagent support, e.g. Copilot, Codex, local Ollama, older Claude): if your runtime cannot spawn subagents, or the spawn fails/times out, read `C:\code\panda\_bmad\render\bmad-build\panda-af65937e8dcd\b1d808c3bf9d40925d5a\compile-epic-context.md` yourself and follow its instructions to produce the same output file.
     ...
     5. **Previous story continuity.** Regardless of which context source succeeded above, scan `C:\code\panda/_bmad-output/implementation-artifacts` for specs from the same epic with `status: done` and a lower story number. Load the most recent one (highest story number below current). Extract its **Code Map**, **Design Notes**, **Spec Change Log**, and **task list** as continuity context for step-02 planning. If no `done` spec is found but an `in-review` spec exists for the same epic with a lower story number, note it to the user and ask whether to load it.
...
4. Multi-goal check (see SCOPE STANDARD). If the intent fails the single-goal criteria:
   ...
   - On **S**: For each deferred goal, append one new entry to `C:\code\panda/_bmad-output/implementation-artifacts/deferred-work.md` using this format. Do not modify existing entries or look for duplicates. Narrow scope to the first-mentioned goal. Continue routing.
     ```markdown
     - source_spec: none
...
5. Route — choose exactly one:
   ...
   If the explicit spec-folder-plus-story-id pair had no matching story file, keep the colocated `spec_file` selected above. Otherwise, derive a valid kebab-case slug from the clarified intent. If the intent references a tracking identifier (story number, issue number, ticket ID), lead the slug with it (e.g. `3-2-digest-delivery`, `gh-47-fix-auth`). If `C:\code\panda/_bmad-output/implementation-artifacts/spec-{slug}.md` already exists: if its status is `draft`, treat it as the same work and resume it (set `spec_file` to that path, **EARLY EXIT** → `C:\code\panda\_bmad\render\bmad-build\panda-af65937e8dcd\b1d808c3bf9d40925d5a\step-02-plan.md`); otherwise append `-2`, `-3`, etc. Set `spec_file` = `C:\code\panda/_bmad-output/implementation-artifacts/spec-{slug}.md`.

   **a) One-shot** — zero blast radius: no plausible path by which this change causes unintended consequences elsewhere. Clear intent, no architectural decisions.

   **EARLY EXIT** → `C:\code\panda\_bmad\render\bmad-build\panda-af65937e8dcd\b1d808c3bf9d40925d5a\step-oneshot.md`

   **b) Plan-code-review** — everything else. When uncertain whether blast radius is truly zero, choose this path.

## NEXT

Read fully and follow `C:\code\panda\_bmad\render\bmad-build\panda-af65937e8dcd\b1d808c3bf9d40925d5a\step-02-plan.md`
